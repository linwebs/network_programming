/*
 * NCYU 109 Network Programming udp multiple server server
...
int main() {
	...
	while (true) {
		recv_status = recvfrom(sd, str, MAXLINE, 0, reinterpret_cast<sockaddr *>(&cli), &cli_len);

		recv_ip = inet_ntoa(cli.sin_addr);
		recv_port = ntohs(cli.sin_port);

		cout << "Receive: " << str << "(" << recv_status << " bytes)";
		cout << " From: " << recv_ip << " (" << recv_port << ")" << endl;
