 ...
 * Exercise 8: udp send many text server
...
int main() {
	...
	while (true) {
		...
		if (strcmp(str_recv, "end") == 0) {
			cout << "file transmission complete. (sum: " << recv_sum << " bytes)" << endl;

			recv_len = recv(sd, str_recv, MAXLINE, 0);

			if (recv_sum != atoi(str_recv)) {
				cout << "Oh... Pkt number mismatched!" << endl;
			} else {
				cout << "Correct, Pkt number matched!" << endl;
			}
			break;
		}
